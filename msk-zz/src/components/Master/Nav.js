...
export default function Nav({ context }) {
    const { width, lang } = useSelector((state) => ({
    ...
    const Component = useMemo(
        () =>
            width < 1024
                ? Drawer
                : forwardRef(({ children }, ref) => <div className="nav-wrapper">{children}</div>),
        [width]
    );
    const drawer = useRef();
                        ...
                        <React.Fragment key={path}>
                            {title && (
                                <div className="nav-title">
                                    {intl.formatMessage({ id: `nav.title.${title}` })}
                                </div>
                            )}
                            <Link disable={disable} onClick={go} path={path}>
                                {label}
                            </Link>
                        </React.Fragment>
                    );
